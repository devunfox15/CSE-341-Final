...
const getById = async (req, res, next) => {
    ...
    try {
        ...
        if (!ObjectId.isValid(orderId)) {
            return next(createError(400, 'Invalid order ID format. Must be a valid ObjectId'));
        }
...
const createorder = async (req, res, next) => {
    ...
    try {
        ...
        if (response.acknowledged) {
            ...
            res.status(500).json(response.error || 'Failed to create order. Please try again.');
        }
    } catch (error) {
        console.log('step 4');
        next(error);
...
const updateorder = async (req, res, next) => {
    ...
    try {
        ...
        if (result.modifiedCount > 0) {
            ...
            return next(createError(500, 'Failed to update order. Please try again.'));
        }
    } catch (error) {
        next(error);
    }
...
const deleteorder = async (req, res, next) => {
    ...
    try {
        ...
        if (result.deletedCount > 0) {
            ...
            return next(createError(500, 'Failed to delete order. Please try again.'));
        }
    } catch (error) {
        next(error);
    }
