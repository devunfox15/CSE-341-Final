...
const getById = async (req, res, next) => {
    ...
    try {
        ...
        const response = await mongodb.getDb().db().collection('products').findOne({ _id: productId });
...
const createProduct = async (req, res, next) => {
    ...
    try {
        ...
        if (response.acknowledged) {
            ...
            res.status(500).json(response.error || 'Failed to create the specified product.');
        }
    } catch (error) {
        next(error);
    }
...
const updateProduct = async (req, res, next) => {
    ...
    try {
        ...
        if (response.modifiedCount > 0) {
            ...
            return next(createError(500, 'Failed to update the specified product.'));
        }
    } catch (error) {
        next(error);
    }
...
const deleteProduct = async (req, res, next) => {
    ...
    try {
        ...
        if (result.deletedCount > 0) {
            ...
            return next(createError(500, 'Failed to delete the specified product.'));
        }
    } catch (error) {
        next(error);
    }
